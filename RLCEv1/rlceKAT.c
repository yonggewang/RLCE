...
#include "rlce.h"
#define OPTIMIZED 0
int RLCEspad(uint8_t bytes[],size_t BLen,
	     uint8_t padded[], size_t paddedLen,
	     RLCE_public_key_t pk,
	     uint8_t randomness[], size_t randLen,
	     uint8_t e0[], size_t e0Len);
int RLCEspadDecode(uint8_t encoded[],size_t encodedLen,
		   uint8_t message[], unsigned long long *mlen,
		   RLCE_private_key_t sk,
		   uint8_t e0[], size_t e0Len);

int RLCEpad(uint8_t bytes[],size_t bytesLen,
	    uint8_t padded[], size_t paddedLen,
	    RLCE_public_key_t pk,
	    uint8_t randomness[], size_t randLen,
	    uint8_t e0[], size_t e0Len);
int RLCEpadDecode(uint8_t encoded[],size_t encodedLen,
		  uint8_t message[], unsigned long long *mlen,
		  RLCE_private_key_t sk,
		  uint8_t e0[], size_t e0Len);
int rangeadd(uint8_t bytes1[], uint8_t bytes2[], size_t bytesize);
...
int getRLCEparameters(size_t para[], size_t scheme, size_t padding) {
  ...
  case 7:
    ...
    para[4]=2;
    para[11]=76;   /* t */
    para[12]=13;   /* max x-degree for Q(x,y) in list-decode: sqrt(2(k-1)n{omega+1 \choose 2}) */
    para[13]=14;   /* L_omega: for list-decoding only */
    para[14]=128; /* kappa-128 */
    para[15]=189; /* u: used for un-recovered msg symbols by RS */
    para[16]=948; /* cipher len in bytes*/
    if (DECODINGMETHOD!=2) {
      para[17]=284636; /* sk bytes for decodingalgorithm 0,1*/
    } else para[17]=173961; /* sk bytes for decoding algorithm 2*/
    para[18]=170091; /* pk bytes */
    para[19]=32; /* random input bytes*/
    switch (padding) {
    case 0: /* 0:RLCEspad */
      para[5]=5420; /*mLen for mediumEncoding */
      para[6]=169;  /* k1 for mediumEncoding */
      para[7]=169;  /* k2 for mediumEncoding */
      para[8]=340; /* k3 for mediumEncoding */
      break;
    case 1: /* 1:RLCEpad  */
      para[5]=5420; /*mLen for mediumEncoding */
      para[6]=614;  /* k1 for mediumEncoding */
      para[7]=32;  /* k2 for mediumEncoding */
      para[8]=32; /* k3 for mediumEncoding */
      break;
    case 2:
      para[5]=4660; /*mLen bEncoding */
      para[6]=145; /* k1 for basicEncoding */
      para[7]=145; /* k2 for basidEncoding */
      para[8]=293; /* k3 for basicEncoding */
      break;
    case 3:
      para[5]=4660; /*mLen bEncoding */
      para[6]=519; /* k1 for basicEncoding */
      para[7]=32; /* k2 for basidEncoding */
      para[8]=32;/* k3 for basicEncoding */
      break;
    case 4:
      para[5]=5771; /*mLen for advancedEncoding */
      para[6]=180; /* k1 for advancedEncoding */
      para[7]=180; /* k2 for advancedEncoding */
      para[8]=362; /* k3 for advancedEncoding */
      break;
    case 5:
      para[5]=5771; /*mLen for advancedEncoding */      
      para[6]=658; /* k1 for advancedEncoding */
      para[7]=60; /* k2 for advancedEncoding */
      para[8]=60;/* k3 for advancedEncoding */
      break;
    default:
  ...
  case 8:
    ...
    para[4]=2;   /* hash type */
    para[11]=73;   /* t */
    para[12]=9;   /* omega for list-decoding */
    para[13]=10;  /* L_omega: for list-decoding only */
    para[14]=128; /* kappa-128 */
    para[15]=117; /* u: used for un-recovered msg symbols by RS */
    para[16]=759; /* cipher len in bytes*/
    if (DECODINGMETHOD!=2) {
      para[17]=166998; /* sk bytes for decodingalgorithm 0,1*/
    } else para[17]=110948; /* sk bytes for decoding algorithm 2*/
    para[18]=107826; /* pk bytes */
    para[19]=32; /* random input bytes*/
    switch (padding) {
    case 0: /* 0:RLCEspad */
      para[5]=4530; /*mLen for mediumEncoding */
      para[6]=141;  /* k1 for mediumEncoding */
      para[7]=141;  /* k2 for mediumEncoding */
      para[8]=285; /* k3 for mediumEncoding */
      break;
    case 1: /* 1:RLCEpad  */
      para[5]=4530; /*mLen for mediumEncoding */
      para[6]=503;  /* k1 for mediumEncoding */
      para[7]=32;  /* k2 for mediumEncoding */
      para[8]=32; /* k3 for mediumEncoding */
      break;
    case 2:
      para[5]=3800; /*mLen bEncoding */
      para[6]=118; /* k1 for basicEncoding */
      para[7]=118; /* k2 for basidEncoding */
      para[8]=239;/* k3 for basicEncoding */
      break;
    case 3:
      para[5]=3800; /*mLen bEncoding */
      para[6]=411; /* k1 for basicEncoding */
      para[7]=32; /* k2 for basidEncoding */
      para[8]=32;/* k3 for basicEncoding */
      break;
    case 4:
      para[5]=4847; /*mLen for advancedEncoding */
      para[6]=151; /* k1 for advancedEncoding */
      para[7]=151; /* k2 for advancedEncoding */
      para[8]=304; /* k3 for advancedEncoding */
      break;
    case 5:
      para[5]=4847; /*mLen for advancedEncoding */      
      para[6]=542; /* k1 for advancedEncoding */
      para[7]=32;  /* k2 for advancedEncoding */
      para[8]=32;  /* k3 for advancedEncoding */
      break;
    default:
  ...
  case 9:
    ...
    para[4]=2;   /* hash type */
    para[11]=108;   /* t */
    para[12]=11;   /* omega for list-decoding */
    para[13]=12;  /* L_omega: for list-decoding only */
    para[14]=192; /* kappa-192 */
    para[15]=285; /* u: used for un-recovered msg symbols by RS */
    para[16]=1513; /* cipher len in bytes*/
    if (DECODINGMETHOD!=2) {
      para[17]=703371; /* sk bytes for decodingalgorithm 0,1*/
    } else para[17]=420946; /* sk bytes for decoding algorithm 2*/
    para[18]=414751; /* pk bytes */
    para[19]=48; /* random input bytes*/
    switch (padding) {
    case 0: /* 0:RLCEspad */
      para[5]=8980; /*mLen for mediumEncoding */
      para[6]=280;  /* k1 for mediumEncoding */
      para[7]=280;  /* k2 for mediumEncoding */
      para[8]=563; /* k3 for mediumEncoding */
      break;
    case 1: /* 1:RLCEpad  */
      para[5]=8980; /*mLen for mediumEncoding */
      para[6]=1027;  /* k1 for mediumEncoding */
      para[7]=48;  /* k2 for mediumEncoding */
      para[8]=48; /* k3 for mediumEncoding */
      break;
    case 2:
      para[5]=7900; /*mLen bEncoding */
      para[6]=246; /* k1 for basicEncoding */
      para[7]=246; /* k2 for basidEncoding */
      para[8]=496;/* k3 for basicEncoding */
      break;
    case 3:
      para[5]=7900; /*mLen bEncoding */
      para[6]=892; /* k1 for basicEncoding */
      para[7]=48; /* k2 for basidEncoding */
      para[8]=48;/* k3 for basicEncoding */
      break;
    case 4:
      para[5]=9500; /*mLen for advancedEncoding */
      para[6]=296; /* k1 for advancedEncoding */
      para[7]=296; /* k2 for advancedEncoding */
      para[8]=596; /* k3 for advancedEncoding */
      break;
    case 5:
      para[5]=9500; /*mLen for advancedEncoding */      
      para[6]=1092; /* k1 for advancedEncoding */
      para[7]=48;  /* k2 for advancedEncoding */
      para[8]=48;  /* k3 for advancedEncoding */
      break;
    default:
  ...
  case 10:
    ...
    para[4]=2;   /* hash type */
    para[11]=107;   /* t */
    para[12]=8;   /* omega for list-decoding */
    para[13]=9;  /* L_omega: for list-decoding only */
    para[14]=192; /* kappa-192 */
    para[15]=175; /* u: used for un-recovered msg symbols by RS */
    para[16]=1195; /* cipher len in bytes*/
    if (DECODINGMETHOD!=2) {
      para[17]=401724; /* sk bytes for decodingalgorithm 0,1*/
    } else para[17]=265324; /* sk bytes for decoding algorithm 2*/
    para[18]=260401; /* pk bytes */
    para[19]=48; /* random input bytes*/
    switch (padding) {
    case 0: /* 0:RLCEspad */
      para[5]=7270; /*mLen for mediumEncoding */
      para[6]=227;  /* k1 for mediumEncoding */
      para[7]=227;  /* k2 for mediumEncoding */
      para[8]=455; /* k3 for mediumEncoding */
      break;
    case 1: /* 1:RLCEpad  */
      para[5]=7270; /*mLen for mediumEncoding */
      para[6]=813;  /* k1 for mediumEncoding */
      para[7]=48;  /* k2 for mediumEncoding */
      para[8]=48; /* k3 for mediumEncoding */
      break;
    case 2:
      para[5]=6200; /*mLen bEncoding */
      para[6]=193; /* k1 for basicEncoding */
      para[7]=193; /* k2 for basidEncoding */
      para[8]=389; /* k3 for basicEncoding */
      break;
    case 3:
      para[5]=6200; /*mLen bEncoding */
      para[6]=679; /* k1 for basicEncoding */
      para[7]=48; /* k2 for basidEncoding */
      para[8]=48;/* k3 for basicEncoding */
      break;
    case 4:
      para[5]=7748; /*mLen for advancedEncoding */
      para[6]=242; /* k1 for advancedEncoding */
      para[7]=242; /* k2 for advancedEncoding */
      para[8]=485; /* k3 for advancedEncoding */
      break;
    case 5:
      para[5]=7748; /*mLen for advancedEncoding */      
      para[6]=873; /* k1 for advancedEncoding */
      para[7]=48;  /* k2 for advancedEncoding */
      para[8]=48;  /* k3 for advancedEncoding */
      break;
    default:
  ...
  case 11:
    ...
    para[4]=2;   /* hash type */
    para[11]=280;   /* t */
    para[12]=26;   /* omega for list-decoding */
    para[13]=34;  /* L_omega: for list-decoding only */
    para[14]=256; /* kappa-256 */
    para[15]=426; /* u: used for un-recovered msg symbols by RS */
    para[16]=2338; /* cipher len in bytes*/
    if (DECODINGMETHOD!=2) {
      para[17]=1382314; /* sk bytes for decodingalgorithm 0,1*/
    } else para[17]=971326; /* sk bytes for decoding algorithm 2*/
    para[18]=926501; /* pk bytes */
    para[19]=60; /* random input bytes*/
    switch (padding) {
    case 0: /* 0:RLCEspad */
      para[5]=10780; /*mLen for mediumEncoding */
      para[6]=336;  /* k1 for mediumEncoding */
      para[7]=336;  /* k2 for mediumEncoding */
      para[8]=676; /* k3 for mediumEncoding */
      break;
    case 1: /* 1:RLCEpad  */
      para[5]=10780; /*mLen for mediumEncoding */
      para[6]=1228;  /* k1 for mediumEncoding */
      para[7]=60;  /* k2 for mediumEncoding */
      para[8]=60; /* k3 for mediumEncoding */
      break;
    case 2:
    ...
    case 3:
      ...
      break;
    case 4:
      para[5]=11872; /*mLen for advancedEncoding */
      para[6]=371; /* k1 for advancedEncoding */
      para[7]=371; /* k2 for advancedEncoding */
      para[8]=742; /* k3 for advancedEncoding */
      break;
    case 5:
      para[5]=11872; /*mLen for advancedEncoding */      
      para[6]=1364; /* k1 for advancedEncoding */
      para[7]=60;  /* k2 for advancedEncoding */
      para[8]=60;  /* k3 for advancedEncoding */
      break;
    default:
  ...
  case 12:
    ...
    para[4]=2;   /* hash type */
    para[11]=262;   /* t */
    para[12]=62;   /* omega for list-decoding */
    para[13]=82;  /* L_omega: for list-decoding only */
    para[14]=256; /* kappa-256 */
    para[15]=295; /* u: used for un-recovered msg symbols by RS */
    para[16]=1898; /* cipher len in bytes*/
    if (DECODINGMETHOD!=2) {
      para[17]=888230; /* sk bytes for decodingalgorithm 0,1*/
    } else para[17]=648100; /* sk bytes for decoding algorithm 2*/
    para[18]=640889; /* pk bytes */
    para[19]=60; /* random input bytes*/
    switch (padding) {
    case 0: /* 0:RLCEspad */
      para[5]=9372; /*mLen for mediumEncoding */
      para[6]=292;  /* k1 for mediumEncoding */
      para[7]=292;  /* k2 for mediumEncoding */
      para[8]=588; /* k3 for mediumEncoding */
      break;
    case 1: /* 1:RLCEpad  */
      para[5]=9372; /*mLen for mediumEncoding */
      para[6]=1052;  /* k1 for mediumEncoding */
      para[7]=60;  /* k2 for mediumEncoding */
      para[8]=60; /* k3 for mediumEncoding */
      break;
    case 2:
      para[5]=6460; /*mLen bEncoding */
      para[6]=202; /* k1 for basicEncoding */
      para[7]=202; /* k2 for basidEncoding */
      para[8]=408;/* k3 for basicEncoding */
      break;
    case 3:
      para[5]=6460; /*mLen bEncoding */
      para[6]=692; /* k1 for basicEncoding */
      para[7]=60; /* k2 for basidEncoding */
      para[8]=60;/* k3 for basicEncoding */
      break;
    case 4:
      para[5]=10334; /*mLen for advancedEncoding */
      para[6]=322; /* k1 for advancedEncoding */
      para[7]=322; /* k2 for advancedEncoding */
      para[8]=648; /* k3 for advancedEncoding */
      break;
    case 5:
      para[5]=10334; /*mLen for advancedEncoding */      
      para[6]=1172; /* k1 for advancedEncoding */
      para[7]=60;  /* k2 for advancedEncoding */
      para[8]=60;  /* k3 for advancedEncoding */
      break;
    default:
  ...
  case 13:
    ...
    para[4]=2;   /* hash type */
    para[11]=11;   /* t */
    para[12]=3;   /* omega for list-decoding */
    para[13]=4;   /* L_omega: for list-decoding only */
    para[14]=128; /* kappa-128 */
    para[15]=7; /* u: used for un-recovered msg symbols by RS */
    para[16]=57; /* cipher len in bytes*/
    if (DECODINGMETHOD!=2) {
    ...
    para[18]=626; /* pk bytes */
    para[19]=6; /* random input bytes*/
    switch (padding) {
    case 0: /* 0:RLCEspad */
      para[5]=310; /*mLen for mediumEncoding */
      para[6]=9;  /* k1 for mediumEncoding */
      para[7]=9;  /* k2 for mediumEncoding */
      para[8]=21; /* k3 for mediumEncoding */
      break;
    case 1: /* 1:RLCEpad  */
      para[5]=310; /*mLen for mediumEncoding */
      para[6]=27;  /* k1 for mediumEncoding */
      para[7]=6;  /* k2 for mediumEncoding */
      para[8]=6; /* k3 for mediumEncoding */
      break;
    case 2:
      para[5]=200; /*mLen bEncoding */
      para[6]=6; /* k1 for basicEncoding */
    ...
    case 3:
      para[5]=200; /*mLen bEncoding */
      para[6]=13; /* k1 for basicEncoding */
      para[7]=6; /* k2 for basidEncoding */
      para[8]=6;/* k3 for basicEncoding */
      break;
    case 4:
      para[5]=343; /*mLen for advancedEncoding */
      para[6]=10; /* k1 for advancedEncoding */
      para[7]=10; /* k2 for advancedEncoding */
      para[8]=23; /* k3 for advancedEncoding */
      break;
    case 5:
      para[5]=343; /*mLen for advancedEncoding */      
      para[6]=31; /* k1 for advancedEncoding */
      para[7]=6; /* k2 for advancedEncoding */
      para[8]=6;/* k3 for advancedEncoding */
      break;
    default:
      return RLCEPADDINGNOTDEFINED;
    }
  ...
  case 14:
    ...
    para[4]=2;   /* hash type */
    para[11]=12;   /* t */
    para[12]=10;   /* omega for list-decoding */
    para[13]=14;   /* L_omega: for list-decoding only */
    para[14]=128; /* kappa-128 */
    para[15]=7; /* u: used for un-recovered msg symbols by RS */
    para[16]=57; /* cipher len in bytes*/
    ...
    switch (padding) {
    case 0: /* 0:RLCEspad */
      para[5]=320; /*mLen for mediumEncoding */
      para[6]=10;  /* k1 for mediumEncoding */
      para[7]=10;  /* k2 for mediumEncoding */
      para[8]=20; /* k3 for mediumEncoding */
      break;
    case 1: /* 1:RLCEpad  */
      para[5]=320; /*mLen for mediumEncoding */
      para[6]=28;  /* k1 for mediumEncoding */
      para[7]=6;  /* k2 for mediumEncoding */
      para[8]=6; /* k3 for mediumEncoding */
      break;
    case 2:
    ...
    case 3:
      ...
      break;
    case 4:
      para[5]=354; /*mLen for advancedEncoding */
      para[6]=11; /* k1 for advancedEncoding */
      para[7]=11; /* k2 for advancedEncoding */
      para[8]=23; /* k3 for advancedEncoding */
      break;
    case 5:
      para[5]=354; /*mLen for advancedEncoding */      
      para[6]=33; /* k1 for advancedEncoding */
      para[7]=6; /* k2 for advancedEncoding */
      para[8]=6;/* k3 for advancedEncoding */
      break;
    default:
...
RLCE_private_key_t RLCE_private_key_init (size_t para[]) {
  ...
  key->para = malloc(PARASIZE * sizeof(size_t));
  int i;
...
RLCE_public_key_t RLCE_public_key_init (size_t para[]) {
  ...
  pk->para = malloc(PARASIZE * sizeof(size_t));
...
int pk2B (RLCE_public_key_t pk, uint8_t pkB[], size_t *blen) {
  ...
  size_t nplusw=pk->para[0]+pk->para[2];
  size_t k=pk->para[1];
  size_t pkLen=k*(nplusw-k);
...
int sk2B (RLCE_private_key_t sk, uint8_t skB[], size_t *blen) {
  ...
  size_t totalFELen=2*w+invSLen+n+k*(n+w-k);
...
RLCE_public_key_t B2pk(const uint8_t binByte[], unsigned long long blen) {
  int i,ret=0;
  size_t scheme=binByte[0] & 0x0F;
  size_t padding=binByte[0]>>4;
  ...
  if ((pk->para[3])==10) ret=B2FE10((uint8_t*)&(binByte[1]), byteLen,FE);
  if ((pk->para[3])==11) ret=B2FE11((uint8_t*)&(binByte[1]), byteLen,FE);
  if (ret<0) return NULL;
...
RLCE_private_key_t B2sk(const uint8_t binByte[], unsigned long long blen) {
  ...
  if ((sk->para[3])==10) ret=B2FE10((uint8_t*)&(binByte[permByteLen+1]), byteLen,FE);
  if ((sk->para[3])==11) ret=B2FE11((uint8_t*)&(binByte[permByteLen+1]), byteLen,FE);
  if (ret<0) return NULL;
  j=0;
  for (i=0; i<w; i++) {
...
int RLCE_key_setup (uint8_t entropy[], int entropylen,
  ...
  uint8_t *randomBytes=calloc(nRB, sizeof(uint8_t));  
  ...
  FILE *f;
  if (n==532) {
    f = fopen("RLCE_KEM_KAT_128A_KEY.txt", "w");
    char * s1="# Algorithm Name: RLCE-KEM-128A-KAT";
    fprintf(f, "%s\n", s1);
    char * s2="# Description of the test: (pk,sk) key generation for RLCE-KEM-128A-KAT";
    fprintf(f, "%s\n", s2);
  }
    
  if (n==630) {
    f = fopen("RLCE_KEM_KAT_128B_KEY.txt", "w");
    char * s1="# Algorithm Name: RLCE-KEM-128B-KAT";
    fprintf(f, "%s\n", s1);
    char * s2="# Description of the test: (pk,sk) key generation for RLCE-KEM-128B-KAT";
    fprintf(f, "%s\n", s2);
  }

  if (n==846) {
    f = fopen("RLCE_KEM_KAT_192A_KEY.txt", "w");
    char * s1="# Algorithm Name: RLCE-KEM-192A-KAT";
    fprintf(f, "%s\n", s1);
    char * s2="# Description of the test: (pk,sk) key generation for RLCE-KEM-192A-KAT";
    fprintf(f, "%s\n", s2);
  }
    
  if (n==1000) {
    f = fopen("RLCE_KEM_KAT_192B_KEY.txt", "w");
    char * s1="# Algorithm Name: RLCE-KEM-192B-KAT";
    fprintf(f, "%s\n", s1);
    char * s2="# Description of the test: (pk,sk) key generation for RLCE-KEM-192B-KAT";
    fprintf(f, "%s\n", s2);
  }
   if (n==1160) {
    f = fopen("RLCE_KEM_KAT_256A_KEY.txt", "w");
    char * s1="# Algorithm Name: RLCE-KEM-256A-KAT";
    fprintf(f, "%s\n", s1);
    char * s2="# Description of the test: (pk,sk) key generation for RLCE-KEM-256A-KAT";
    fprintf(f, "%s\n", s2);
  }
    
  if (n==1360) {
    f = fopen("RLCE_KEM_KAT_256B_KEY.txt", "w");
    char * s1="# Algorithm Name: RLCE-KEM-256B-KAT";
    fprintf(f, "%s\n", s1);
    char * s2="# Description of the test: (pk,sk) key generation for RLCE-KEM-256B-KAT";
    fprintf(f, "%s\n", s2);
  }
    
  char * s3="# Other parameters: ";
  char * s4="# ALL VALUES ARE IN HEX FORMAT";
  char * s5="##########################################################################";
  
  fprintf(f, "%s", s3);
  fprintf(f, "n=%d, k=%d, t=%d, w=%d, m=%d\n", n,k,t,w,m);
  fprintf(f, "%s\n", s4);
  fprintf(f, "%s\n", s5);
  
  fprintf(f, "Input Values:\nentropy_input for randombytes_init() = ae7ebe062971f5eb32e5b21444750785de816595ad2cbe80a209c8f8ab04b546\nOutput of randombytes() = ");
  for (i=0; i<entropylen; i++) fprintf(f,"%02x", entropy[i]);
  fprintf(f, "\nnonce = 5e7d69e187577b0433eee8eab9f77731 (hard coded in crypto_kem_keypair())\n");
  ...
  if (DRBG==2) {
    ...
    uint8_t *mgfseed=calloc(mgfseedLen, sizeof(uint8_t));
    memcpy(mgfseed, entropy, entropylen*sizeof(uint8_t));
    memcpy(&mgfseed[entropylen], pers, perlen*sizeof(uint8_t));
    memcpy(&mgfseed[entropylen+perlen], addS, addlen*sizeof(uint8_t));
    RLCE_MGF512(mgfseed,mgfseedLen,randomBytes, nRB);     
  }

  /* KAT */
  fprintf(f, "randBytess = ");  
  for (i=0; i<nRB; i++) fprintf(f,"%02x", randomBytes[i]);
  fprintf(f,"\n");
  /* KAT */
  ...
  if (LISTDECODE==0) generator=genPolyTable(n-k); //getGenPoly(n-k, generator, m);
  matrix_t G= NULL;
  matrix_t G2= NULL;
  if (OPTIMIZED==1) {
    ...
    if (LISTDECODE==0) {
      ...
      for (i=0; i<k; i++) {
	for (j=0; j<n;j++) optG->data[j][i]= GF_mul(GF_exp((i*j)%(fieldSize(m)-1),m), grsvec[j] ,m); 
      }
    }
    ...
    if (ret<0) return ret;
      ...
      /* KAT */
  fprintf(f, "\ngenerator(x) = (");
  for (i=0; i<n-k; i++) fprintf(f,"%04x,", generator->coeff[i]);
  fprintf(f,"%04x)\n", generator->coeff[n-k]);
  
  fprintf(f, "\ngrs = (");  
  for (i=0; i<n-1; i++) fprintf(f,"%04x,", grsvec[i]);
  fprintf(f,"%04x)\n", grsvec[n-1]);

  for (i=0; i<w; i++) {
    fprintf(f, "\nA[%d] = ",i);
    fprintf(f, "(%04x,%04x,%04x,%04x)", A->A[i]->data[0][0],A->A[i]->data[0][1],A->A[i]->data[1][0],A->A[i]->data[1][1]);
  }

  fprintf(f, "\n");
  for (i=0;i<k; i++) {
    fprintf(f, "\n%d-th row of R = (",i);
    for (j=0; j<w-1;j++) fprintf(f, "%04x,",R->data[i][j]);
    fprintf(f, "%04x)",R->data[i][w-1]);
  }

  fprintf(f, "\n\nP1 = (");
  for (i=0; i<n-1;i++) fprintf(f, "%04x,",per1->data[i]);
  fprintf(f, "%04x)",per1->data[n-1]);

  fprintf(f, "\n\nP2 = (");
  for (i=0; i<n+w-1;i++) fprintf(f, "%04x,",per2->data[i]);
  fprintf(f, "%04x)",per1->data[n+w-1]);
  fprintf(f, "\n");
  /* KAT */
  ...
  /* KAT */
  for (i=0; i<k; i++) {
    fprintf(f, "%d-th row of X =(",i);
    for (j=0; j<(sk->S)->numC-1;j++) fprintf(f, "%04x,",(sk->S)->data[i][j]);
    fprintf(f, "%04x)\n",(sk->S)->data[i][(sk->S)->numC-1]);
  }

  for (i=0; i<k; i++) {
    fprintf(f, "%d-th row of G =(",i);
    for (j=0; j<(sk->G)->numC-1;j++) fprintf(f, "%04x,",(sk->G)->data[i][j]);
    fprintf(f, "%04x)\n",(sk->G)->data[i][(sk->G)->numC-1]);
  }

  size_t sklen =sk->para[17];
  uint8_t *skB=calloc(sklen, sizeof(uint8_t));
  sk2B(sk, skB, &sklen);
  fprintf(f, "\n\nOutput Values:\n\n");
  fprintf(f, "\nsk = ");
  for (i=0; i<sklen; i++) fprintf(f, "%02x", skB[i]);
  fprintf(f, "\n");
  free(skB);

  size_t pklen =pk->para[18];
  uint8_t *pkB=calloc(pklen, sizeof(uint8_t));
  pk2B(pk,pkB,&pklen);
  fprintf(f, "\npk = ");
  for (i=0; i<pklen; i++) fprintf(f, "%02x", pkB[i]);
  fprintf(f, "\n");
  free(pkB);
...
int RLCE_encrypt(uint8_t msg[], uint8_t entropy[], size_t entropylen,
  ...
  uint8_t pers[] ="PQENCRYPTIONRLCEver1";
  int perslen = sizeof(pers)-1;
  uint8_t add[]="GRSbasedPQEncryption0";
  ...
  nRB=nRB0+nRB1;
  uint8_t * randBytes;
  randBytes = (uint8_t *) calloc(nRB, sizeof(uint8_t));
  uint8_t * padrand;
  padrand =  (uint8_t *) calloc(pk->para[8], sizeof(uint8_t));
  hash_drbg_state_t drbgState=NULL;
  ctr_drbg_state_t ctrdrbgState=NULL;
  drbg_Input_t drbgInput=NULL;

  if ((CTRPADDRBG==0)&&(DRBG!=2)){
    uint8_t nonceAppend[]="RLCEencNonceVer1";
    int nonceAppendlen = sizeof(nonceAppend)-1;
    uint8_t noncenew[noncelen+nonceAppendlen];
  ...
  if (DRBG==2) {
    memcpy(mgfseed, entropy, entropylen*sizeof(uint8_t));
    memcpy(&mgfseed[entropylen], pers, perslen*sizeof(uint8_t));
    memcpy(&mgfseed[entropylen+perslen], add, addlen*sizeof(uint8_t));
  }
  ...
  FILE *f;
  if (n==532) {
    f = fopen("RLCE_KEM_KAT_128A_ENCAPSULATION.txt", "w");
    char * s1="# Algorithm Name: RLCE-KEM-128A-KAT";
    fprintf(f, "%s\n", s1);
    char * s2="# Description of the test: encapsulation process for RLCE-KEM-128A-KAT";
    fprintf(f, "%s\n", s2);
  }
    
  if (n==630) {
    f = fopen("RLCE_KEM_KAT_128B_ENCAPSULATION.txt", "w");
    char * s1="# Algorithm Name: RLCE-KEM-128B-KAT";
    fprintf(f, "%s\n", s1);
    char * s2="# Description of the test: encapsulation process for RLCE-KEM-128B-KAT";
    fprintf(f, "%s\n", s2);
  }

  if (n==846) {
    f = fopen("RLCE_KEM_KAT_192A_ENCAPSULATION.txt", "w");
    char * s1="# Algorithm Name: RLCE-KEM-192A-KAT";
    fprintf(f, "%s\n", s1);
    char * s2="# Description of the test: encapsulation process for RLCE-KEM-192A-KAT";
    fprintf(f, "%s\n", s2);
  }
    
  if (n==1000) {
    f = fopen("RLCE_KEM_KAT_192B_ENCAPSULATION.txt", "w");
    char * s1="# Algorithm Name: RLCE-KEM-192B-KAT";
    fprintf(f, "%s\n", s1);
    char * s2="# Description of the test: encapsulation process for RLCE-KEM-192B-KAT";
    fprintf(f, "%s\n", s2);
  }
   if (n==1160) {
    f = fopen("RLCE_KEM_KAT_256A_ENCAPSULATION.txt", "w");
    char * s1="# Algorithm Name: RLCE-KEM-256A-KAT";
    fprintf(f, "%s\n", s1);
    char * s2="# Description of the test: encapsulation process for RLCE-KEM-256A-KAT";
    fprintf(f, "%s\n", s2);
  }
    
  if (n==1360) {
    f = fopen("RLCE_KEM_KAT_256B_ENCAPSULATION.txt", "w");
    char * s1="# Algorithm Name: RLCE-KEM-256B-KAT";
    fprintf(f, "%s\n", s1);
    char * s2="# Description of the test: encapsulation process for RLCE-KEM-256B-KAT";
    fprintf(f, "%s\n", s2);
  }
  ...
  fprintf(f, "ss = ");
  for (i=0; i<64; i++) fprintf(f,"%02x", msg[i]);
  size_t pklen =pk->para[18];
  uint8_t *pkB=calloc(pklen, sizeof(uint8_t));
  pk2B(pk,pkB,&pklen);
  fprintf(f, "\npk = ");
  ...
  uint8_t *paddedMSG=calloc(paddedLen, sizeof(uint8_t));
  while (repeat) { /* this loop makes sure that errors will not be zero */
    ...
    /* KAT */
    fprintf(f, "ctr=%d\n\nrandBytes = ", ctr);
    for (i=0; i<nRB; i++) fprintf(f, "%02x", randBytes[i]);

    fprintf(f, "\n\nerrLocation = (");
    for (i=0;i<t-1;i++) fprintf(f, "%04x,", errLocation[i]);
    fprintf(f, "%04x)", errLocation[t-1]);

    fprintf(f, "\n\npaddedMSG = ");
    for (i=0;i<paddedLen;i++) fprintf(f, "%02x", paddedMSG[i]);
    
    fprintf(f, "\n\nerrValue = (");
    for (i=0;i<t-1;i++) fprintf(f, "%04x,", errValue->data[i]);
    fprintf(f, "%04x)", errValue->data[t-1]);
    /* KAT */
  }
  ...
  /* KAT */
  fprintf(f, "\n\n\nOutput Values:\nc = ");
  for (i=0;i<clen[0];i++) fprintf(f, "%02x", cipher[i]);
  fprintf(f, "\n");
  fclose(f);
  /* KAT */
...
int RLCE_decrypt(uint8_t cipher[], unsigned long long clen, RLCE_private_key_t sk, uint8_t msg[],
  ...
  FILE *f;
  if (n==532) {
    f = fopen("RLCE_KEM_KAT_128A_DECAPSULATION.txt", "w");
    char * s1="# Algorithm Name: RLCE-KEM-128A-KAT";
    fprintf(f, "%s\n", s1);
    char * s2="# Description of the test: decapsulation process for RLCE-KEM-128A-KAT";
    fprintf(f, "%s\n", s2);
  }
    
  if (n==630) {
    f = fopen("RLCE_KEM_KAT_128B_DECAPSULATION.txt", "w");
    char * s1="# Algorithm Name: RLCE-KEM-128B-KAT";
    fprintf(f, "%s\n", s1);
    char * s2="# Description of the test: decapsulation process for RLCE-KEM-128B-KAT";
    fprintf(f, "%s\n", s2);
  }

  if (n==846) {
    f = fopen("RLCE_KEM_KAT_192A_DECAPSULATION.txt", "w");
    char * s1="# Algorithm Name: RLCE-KEM-192A-KAT";
    fprintf(f, "%s\n", s1);
    char * s2="# Description of the test: decapsulation process for RLCE-KEM-192A-KAT";
    fprintf(f, "%s\n", s2);
  }
    
  if (n==1000) {
    f = fopen("RLCE_KEM_KAT_192B_DECAPSULATION.txt", "w");
    char * s1="# Algorithm Name: RLCE-KEM-192B-KAT";
    fprintf(f, "%s\n", s1);
    char * s2="# Description of the test: decapsulation process for RLCE-KEM-192B-KAT";
    fprintf(f, "%s\n", s2);
  }
   if (n==1160) {
    f = fopen("RLCE_KEM_KAT_256A_DECAPSULATION.txt", "w");
    char * s1="# Algorithm Name: RLCE-KEM-256A-KAT";
    fprintf(f, "%s\n", s1);
    char * s2="# Description of the test: decapsulation process for RLCE-KEM-256A-KAT";
    fprintf(f, "%s\n", s2);
  }
    
  if (n==1360) {
    f = fopen("RLCE_KEM_KAT_256B_DECAPSULATION.txt", "w");
    char * s1="# Algorithm Name: RLCE-KEM-256B-KAT";
    fprintf(f, "%s\n", s1);
    char * s2="# Description of the test: decapsulation process for RLCE-KEM-256B-KAT";
    fprintf(f, "%s\n", s2);
  }
  ...
  fprintf(f, "Input Values:\ncipher = ");
  for (i=0; i<clen; i++) fprintf(f,"%02x", cipher[i]);
  ...
  fprintf(f, "\n\nIntermediate Values:\n\n");
  fprintf(f, "C3 = (");
  for (i=0; i<n-1; i++) fprintf(f, "%04x, ", cp[i]);
  fprintf(f, "%04x)", cp[n-1]);
  /* KAT */
  ...
  /* KAT */
  fprintf(f, "code = (");
  for (i=0; i<n-1; i++) fprintf(f, "%04x, ", decodedWord->coeff[zeroLen+i]);
  fprintf(f, "%04x)", decodedWord->coeff[zeroLen+n-1]);
  /* KAT */
  ...
  /* KAT */
  fprintf(f, "(I_0, ..., I_{u-1}) = (");
  for (i=0; i<remDim-1; i++) fprintf(f, "%04x, ", unknownIndex[i]);
  fprintf(f, "%04x)", unknownIndex[remDim-1]);  
  fprintf(f, "\n(J_0, ..., J_{k-u-1}) = (");
  for (i=0; i<errClearPos-1; i++) fprintf(f, "%04x, ", knownIndex[i]);
  fprintf(f, "%04x)\n", knownIndex[errClearPos-1]);
  /* KAT */
  ...
  size_t errLocation[t];
  memset(errLocation, 0, t*sizeof(size_t));
  vector_t errValue=vec_init(t);
  ...
  if (OPTIMIZED==1) {
	...
	if (tmpidx<t) {
      ...
      /* KAT */
  fprintf(f, "msg = (");
  for (i=0; i<k-1; i++) fprintf(f, "%04x, ", dest[i]);
  fprintf(f, "%04x)\n", dest[k-1]);
  
  fprintf(f, "\ncipher'' = (");
  for (i=0; i<k; i++) fprintf(f, "%04x, ", dest[i]);
  for (i=0; i<nplusw-1; i++) fprintf(f, "%04x, ", cipherNoError->data[i]);  
  fprintf(f, "%04x)\n", cipherNoError->data[nplusw-1]);
  fprintf(f, "\ne0 = (");
  for (i=0; i<t-1; i++) fprintf(f, "%04x, ", errLocation[i]);
  fprintf(f, "%04x)\n", errLocation[t-1]);
  ...
  free(cipherB4A);
  free(dest);
  uint8_t paddedMSG[paddedLen]; /* padded msg k1+k1+k2 bytes */
  ...
  if (ret<0) return ret;
   ...
   /* KAT */
  fprintf(f, "\nFE = (");
  for (i=0; i<FE_vec->size-1; i++) fprintf(f, "%04x, ",FE_vec->data[i]);
  fprintf(f, "%04x)\n", FE_vec->data[FE_vec->size-1]);
  ...
  int e0Len = 4*t;
  uint8_t e0[e0Len];/* bytes used for padding purpose */
  for (i=0; i<t; i++) {
  ...
  if ((sk->para[9] == 2) || (sk->para[9] == 3)) { /* bEncoding */
   ...
   /* KAT */
  fprintf(f, "\nmesssage = ");
  for (i=0; i<mlen[0]; i++) fprintf(f, "%02x",msg[i]);
...
uint8_t* rlceReadFile(char* filename, unsigned long long *blen, int hex) {
  ...
  fclose(f);
  if (hex==0) return (uint8_t*) buffer;
  if ((blen[0]%2)>0) return NULL;
  blen[0] = blen[0]/2;
  char buf[10];
  uint8_t *hexBin=NULL; 
  hexBin=calloc(blen[0], sizeof(uint8_t));
  int count;
...
RLCE_private_key_t readSK(char* filename, int hex) {
  ...
  uint8_t* binByte=rlceReadFile(filename, &blen, hex);
...
int RLCEspad(uint8_t bytes[],size_t bytesLen,
	     uint8_t padded[], size_t paddedLen,
	     RLCE_public_key_t pk,
	     uint8_t randomness[], size_t randLen,
	     uint8_t e0[], size_t e0Len) {
  ...
  } else for (i=0;i<k1+k2;i++) padded[i]=padded[i]^h2re0[i];
  return 0;
}
...
int RLCEspadDecode(uint8_t encoded[],size_t encodedLen,
  ...
  uint8_t randomness[k3];
  memcpy(randomness, &encoded[k1+k2],k3);
  size_t alpha=8*(k1+k2+k3)-sk->para[5]; 
  uint8_t mask = 0xFF << alpha;
  randomness[k3-1] &= mask; /* set the last alpha bits as zero */
  uint8_t re0[k3+e0Len];
  memcpy(re0, randomness,k3);
  ...
  } else for (i=0;i<k1+k2;i++) encoded[i] = encoded[i] ^h2re0[i];
...
int RLCEpadDecode(uint8_t encoded[],size_t encodedLen,
  ...
  } else for (i=0; i<k3; i++) randomness[i]=encoded[k1+k2+i] ^  h3mh1Ph2[i];
  size_t alpha=8*(k1+k2+k3)-sk->para[5]; 
  uint8_t mask = 0xFF << alpha;
  randomness[k3-1] &= mask; /* set the last alpha bits as zero */
  uint8_t re0[k3+e0Len];
  ...
  if (OPTIMIZED==1) {
    rangeadd(h2re0, encoded,k1+k2);
  } else for (i=0; i<k1+k2; i++) encoded[i] ^= h2re0[i];
...
void hex2char(char hex[], uint8_t hexChar[], int charlen){
  int i=0;
  char buf[8];
  for(i = 0; i < charlen; i++) {
