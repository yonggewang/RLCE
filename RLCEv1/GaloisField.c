...
static short **GFmulTable[17]={NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL};
static short **GFdivTable[17]={NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL};
...
field_t GF_fexp(field_t x, int y, int m) {
  ...
  if (y==0) return x;
  //result = (GFlogTable[m][x] + y)%fieldOrder[m];
...
void GF_mulvec(field_t x, field_t vec[], field_t dest[],int dsize, unsigned int m) {
  ...
  for (i=0; i<dsize; i++) {
    if(vec[i]==0) {
      ...
      //tmp=(xlog+GFlogTable[m][vec[i]])%fieldOrder[m];
...
void GF_divvec(field_t vec1[],field_t vec2[], int vsize, unsigned int m) {
  ...
  //vec1[i]=GFexpTable[m][(GFlogTable[m][vec1[i]]+fieldOrder[m]-GFlogTable[m][vec2[i]])%fieldOrder[m]];
  return;
}
...
int GF_vecreversemul(field_t vec1[],field_t vec2[],int vsize,int m) {
  ...
  GF_init_logexp_table(m);
  for (i=0; i< vsize; i++) {
    if ((vec1[i]!=0) && (vec2[vsize-i]!=0)) {
      //d^==GFexpTable[m][(GFlogTable[m][vec1[i]]+GFlogTable[m][vec2[vsize-i]])%fieldOrder[m]];
...
int getMatrixAandAinv(matrixA_t mat, matrixA_t matInv,
  ...
  if (GFMULTAB==1) {
	...
	b= GFmulTable[m][randomElements[j+1]][randomElements[j+2]];
	det=a^b;
	if (det==0) j++;
  ...
  for (i=0; i< mat->size; i++) {
    ...
    while (det==0) {
      ...
      b=GFexpTable[m][GFlogTable[m][randomElements[j+1]]+GFlogTable[m][randomElements[j+2]]]; 
      det=a^b;
      if (det==0) j++; 
      if (j+4 >randB) return NEEDNEWRANDOMSEED;
    }
    ...
    (mat->A[i])->data[1][0]= randomElements[j+2];
    (mat->A[i])->data[1][1]= randomElements[j+3];
    //a=fieldOrder[m]-GFlogTable[m][det];
    (matInv->A[i])->data[0][0] = GFexpTable[m][GFlogTable[m][randomElements[j+3]]-GFlogTable[m][det]];
    (matInv->A[i])->data[1][0] = GFexpTable[m][GFlogTable[m][randomElements[j+2]]-GFlogTable[m][det]];
    j=j+4;
  }
  return 0;
};
...
void GF_vecvecmul(field_t v1[], field_t v2[], field_t v3[], int vsize, unsigned int m)  {
  ...
  for (i=0;i<vsize;i++) {
    if ((v1[i]!=0) && (v2[i]!=0)) {
      //v3[i]=GFexpTable[m][(GFlogTable[m][v1[i]]+GFlogTable[m][v2[i]])%fieldOrder[m]];
...
void GF_evalpoly0(int log, poly_t p, field_t input[], field_t output[], int size, int m) {
  int i,j;
  GF_init_mult_table(m);
  if (log ==1)
    for (j=0;j<size;j++) input[j]=GFexpTable[m][input[j]];
  int d = p->deg;
  for (i=0; i<size; i++) output[i]=p->coeff[d];
  for (i=d-1; i>=0; i--) {
    for (j=0;j<size;j++) output[j]=GFmulTable[m][input[j]][output[j]];
...
void GF_rsgenerator2optG(matrix_t optG, poly_t generator, field_t grsE[], int m) {
  ...
  for (i=0; i<optG->numC; i++) {
	...
	optG->data[j][i]=
	  //GFexpTable[m][(GFlogTable[m][generator->coeff[j-i]]+GFlogTable[m][grsE[j]])%(fieldOrder[m])];
...
void getGenPoly (int deg, poly_t g, int m) {
  ...
  for (i = 1; i <= deg; i++) {
    ...
    for (j=0;j<=g->deg; j++) {
      if (g->coeff[j] !=0) {
	//g->coeff[j]=GFexpTable[m][(GFlogTable[m][g->coeff[j]]+i)%(fieldOrder[m])];
...
void GF_mulAinv(field_t cp[], matrixA_t A, field_t C1[], int m) {
  ...
  for (j=0; j<A->size; j++) {
    ...
    b2=0;
    if (cp[2*j]!=0)
      //b1=GFexpTable[m][(GFlogTable[m][cp[2*j]]+GFlogTable[m][(A->A[j])->data[0][0]])%(fieldOrder[m])];
      b1=GFexpTable[m][GFlogTable[m][cp[2*j]]+GFlogTable[m][(A->A[j])->data[0][0]]];
    if (cp[2*j+1]!=0)
      //b2=GFexpTable[m][(GFlogTable[m][cp[2*j+1]]+GFlogTable[m][(A->A[j])->data[1][0]])%(fieldOrder[m])];
...
field_t GF_div(field_t x, field_t y, int m) {
  ...
  result = GFlogTable[m][x]+fieldOrder[m]-GFlogTable[m][y];  
  return GFexpTable[m][result];
}
...
field_t GF_inv(field_t y, int m){
  if (y == field_zero()) return -1;
  return GF_div(field_unit(), y, m);
}

void GF_print_log (int m) {
  int i;
  GF_init_logexp_table(m);
  for (i=0; i<fieldSize(m); i++) printf("0x%03x,",GF_log(i,m));
  printf("\n");
}
