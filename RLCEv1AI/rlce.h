...
typedef struct ctr_drbg_state {
  ...
  unsigned short
      seedlen; /* 256/8 for AES-128, 320/8 for AES-192, and 384/8 for AES-256 */
...
int rlceWriteFile(char *filename, unsigned char bytes[],
                  unsigned long long blen, int hex);
...
int ctr_DRBG_Instantiate_algorithm(ctr_drbg_state_t drbgState,
...
int GGIFFT(int i, vector_t base, field_t beta, vector_t output, poly_t r,
           matrix_t smat, int m);
...
int RLCE_key_setup(unsigned char entropy[], int entropylen,
                   unsigned char nonce[], int noncelen, RLCE_public_key_t pk,
...
int getRLCEparameters(unsigned int para[], unsigned int scheme,
...
int RLCE_encrypt(unsigned char msg[], unsigned long long mLen,
                 ...
                 RLCE_public_key_t pk, unsigned char cipher[],
                 unsigned long long *clen);
int RLCE_decrypt(unsigned char cipher[], unsigned long long clen,
                 RLCE_private_key_t sk, unsigned char msg[],
...
void GF_vec_winograd(field_t x, field_t vec[], matrix_t B, matrix_t tmp,
                     unsigned int m);
...
extern int GF_addvec(field_t vec1[], field_t vec2[], field_t vec3[],
                     int vecSize);
...
void GF_evalpoly(int log, poly_t p, field_t input[], field_t output[], int size,
                 int m);
void GF_rsgenerator2optG(matrix_t optG, poly_t generator, field_t randE[],
                         int m);
void GF_vecvecmul(field_t v1[], field_t v2[], field_t v3[], int vsize,
                  unsigned int m);
void getGenPoly(int deg, poly_t g, int m);
void rootsLocation(field_t rts[], int nRts, field_t eLoc[], field_t rtLog[],
                   int m);
...
#define GF_div(x, y, m)                                                        \
  ((x) ? GFexpTable[m][GFlogTable[m][x] + fieldOrder[m] - GFlogTable[m][y]] : 0)
#define GF_tablemul(x, y, m) GFmulTable[m][x][y]
#define GF_mulx(x, y, m)                                                       \
  ((y) ? GFexpTable[m][GFlogTable[m][x] + GFlogTable[m][y]] : 0)
...
int getRandomBytes(unsigned char seed[], int seedSize, unsigned char pers[],
                   int persSize, unsigned char output[], int outputSize,
                   int shatype);
