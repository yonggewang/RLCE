...
int berlekamp_massey_original(poly_t syndrome, poly_t C, int m) {
  /* this process implements the algorithm in: 
     ...
     IEEE Trans. Information Theory 15(1):122–127 
  */
...
poly_t rs_decode(int decoder, poly_t code, int codelen, int codedim, field_t eLocation[], int m) {
  ...
  if (OPTIMIZED == 0) {
    ...
    /*
    //numRoots= find_roots(lambda, lambdaRoots, eLocation, m);
    numRoots= find_roots_FFT(lambda,lambdaRoots, m);
    //printf("num of roots: %d\n", numRoots);
    field_t* lambdaRootsLog=calloc(numRoots, sizeof(field_t));
    rootsLocation(lambdaRoots, numRoots, eLocation, lambdaRootsLog,m);    
    field_t *omegaoutput=calloc(numRoots, sizeof(field_t));
    field_t *lanmdaDoutput=calloc(numRoots, sizeof(field_t));
    GF_evalpoly(omega, lambdaRootsLog, omegaoutput, numRoots, m);
    GF_evalpoly(lambdaDerivative, lambdaRootsLog, lanmdaDoutput,numRoots,  m);
    GF_divvec(omegaoutput,lanmdaDoutput,numRoots, m);
    free(lambdaRootsLog);
    */
