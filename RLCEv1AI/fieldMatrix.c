...
void matrix_zero(matrix_t A) {
  int i;
  for (i = 0; i < A->numR; i++)
    memset(A->data[i], 0, (A->numC) * sizeof(field_t));
  return;
}
...
int matrix_vec_mat_mul_winograd(field_t V[], matrix_t B, field_t dest[],
                                int dsize, unsigned int m) {
  int i, j;
  field_t x = 0;
  field_t *desttmp;
  desttmp = calloc(dsize, sizeof(field_t));
  memset(dest, 0, (dsize) * sizeof(field_t));
  for (i = 0; i < (B->numR) / 2; i++)
    x ^= GF_mul(V[2 * i], V[2 * i + 1], m);
  ...
  int odd = (B->numR) % 2;
  for (j = 0; j < (B->numR) / 2; j++)
    GF_addvec(tmp->data[j], dest, NULL, dsize);
  if (odd > 0) {
...
int matrix_vec_mat_mul_standard(field_t V[], int vsize, matrix_t B,
                                field_t dest[], int dsize, unsigned int m) {
  if ((vsize > B->numR) || (B->numC < dsize))
    return VECMATRIXMULERROR;
  int i;
  memset(dest, 0, dsize * sizeof(field_t));
  for (i = 0; i < vsize; i++) {
    if (V[i]) {
      GF_muladdvec(V[i], B->data[i], dest, dsize, m);
    }
  }
  return 0;
}
...
int matrix_col_permutation(matrix_t A, vector_t per) {
  /* per should contain a permutation of
     0, 1, ..., A->numC */
  if (per->size != A->numC)
    return MATRIXCOLPERERROR;
  field_t tmp[A->numC];
  int i, j;
  for (i = 0; i < A->numR; i++) {
    memcpy(tmp, A->data[i], (A->numC) * sizeof(field_t));
    for (j = 0; j < A->numC; j++)
      A->data[i][j] = tmp[per->data[j]];
  }
  return 0;
}
...
int matrix_inv_standard(matrix_t G, matrix_t result, int m) {
  ...
  for (i = 0; i < n; i++)
    memcpy(mat->data[i], G->data[i], n * sizeof(field_t));
  for (i = 0; i < n; i++)
    mat->data[i][n + i] = 1;
  ret = matrix_echelon(mat, m);
  for (i = 0; i < n; i++)
    memcpy(result->data[i], &(mat->data[i][n]), n * sizeof(field_t));
  matrix_free(mat);
  return ret;
}
...
int matrix_echelon(matrix_t G, int m) {
  ...
  field_t *tmp;
  field_t c;
  // field_t *tmprow = calloc(G->numC, sizeof(field_t)); // Removed: using fused
  // muladd
  for (j = 0; j < n; j++) { /* converting first half to I */
    if (G->data[j][j] == 0) {
      temp = j;
      while ((temp < G->numR) && (G->data[temp][j] == 0))
        temp++;
      if (temp == n) {
        // free(tmprow);
        return -j;
    ...
    for (i = 0; i < n; i++) {
      if (i != j) {
        c = G->data[i][j];
        if (c != field_zero()) {
          GF_muladdvec(c, &(G->data[j][j]), &(G->data[i][j]), (G->numC) - j, m);
        }
      }
    }
  }
...
int randomBytes2FE(unsigned char randomBytes[], int nRB, field_t output[],
                   int outputSize, int m) {
  vector_t Vec;
...
int getShortIntegers(unsigned char randB[], int nRB, unsigned short output[],
                     int outputSize) {
  int i;
...
int getRandomBytes(unsigned char seed[], int seedSize, unsigned char pers[],
                   int persSize, unsigned char output[], int outputlen,
                   int cryptotype) {
  int ret = 0;
  if (cryptotype < 3) {
    ...
    drbgInput =
        drbgInput_init(seed, seedSize, nonce, 8, pers, persSize, add, 0);
    ...
    if (seedlen > drbgState->seedlen)
      seedlen = drbgState->seedlen;
...
int RLCE_MGF512(unsigned char mgfseed[], int mgfseedLen, unsigned char mask[],
                int maskLen) {
  ...
  for (i = 0; i < m; i++) {
    for (j = 3; j >= 0; j--)
      seed[mgfseedLen + j] = (0xFF & (i >> ((3 - j) * 8)));
    sha512_md(seed, mgfseedLen + 4, hash512);
    for (j = 0; j < 64; j++)
      mask[i * 64 + j] = (hash512[j / 8] >> (56 - (j % 8) * 8)) & 0xFF;
  }
  if (r > 0) {
    for (j = 3; j >= 0; j--)
      seed[mgfseedLen + j] = (0xFF & (m >> ((3 - j) * 8)));
    sha512_md(seed, mgfseedLen + 4, hash512);
    for (j = 0; j < r; j++)
      mask[m * 64 + j] = (hash512[j / 8] >> (56 - (j % 8) * 8)) & 0xFF;
  }
  return 0;
}

int RLCE_MGF(unsigned char mgfseed[], int mgfseedLen, unsigned char mask[],
             int maskLen, int shatype) {
  unsigned char seed[mgfseedLen + 4];
  memcpy(seed, mgfseed, mgfseedLen);
  ...
  if ((shatype == 0) || (shatype == 1)) {
    ...
    for (i = 0; i < m; i++) {
      ...
      for (j = 0; j < (4 * hashSize); j++)
        mask[i * (4 * hashSize) + j] =
            (hash[i / 4] >> (24 - (i % 4) * 8)) & 0xFF;
    }
    if (r > 0) {
      for (j = 3; j >= 0; j--)
        seed[mgfseedLen + j] = (0xFF & m >> ((3 - j) * 8));
...
int matrix_mul_strassen_aux(matrix_t A, matrix_t B, matrix_t C, int m) {
  ...
  if ((row <= STRASSENCONST) || (col <= STRASSENCONST) ||
      (midRC <= STRASSENCONST)) {
  ...
  for (i = 0; i < rowHf - rowRem; i++) {
    ...
    GF_addvec(M1->data[i], M2->data[i], &(C->data[rowHf + i][colHf]),
              colHf - colRem);
...
int matrix_mul_strassenSLOW(matrix_t A, matrix_t B, matrix_t C, int m) {
  int i, ret, scon = 1;
  while (STRASSENCONST * scon < A->numC)
    scon = 2 * scon;
  ...
  for (i = 0; i < A->numR; i++)
    memcpy(SA->data[i], A->data[i], (A->numC) * sizeof(field_t));
  for (i = 0; i < B->numR; i++)
    memcpy(SB->data[i], B->data[i], (B->numC) * sizeof(field_t));
  ret = matrix_mul_strassen_aux(SA, SB, SC, m);
  if (ret < 0)
    return ret;
  for (i = 0; i < C->numR; i++)
    memcpy(C->data[i], SC->data[i], (C->numC) * sizeof(field_t));
  matrix_free(SA);
  matrix_free(SB);
  matrix_free(SC);
  return 0;
...
int matrix_mul_strassen(matrix_t A, matrix_t B, matrix_t C, int m) {
  ...
  for (i = 0; i < rowHf; i++) {
    ...
    memcpy(A12->data[i], &(A->data[i][midRCf]),
           (midRCf - midRCRem) * sizeof(field_t));
  }
  for (i = 0; i < rowHf - rowRem; i++) {
    memcpy(A21->data[i], A->data[rowHf + i], midRCf * sizeof(field_t));
    memcpy(A22->data[i], &(A->data[rowHf + i][midRCf]),
           (midRCf - midRCRem) * sizeof(field_t));
  }
  for (i = 0; i < midRCf; i++) {
    memcpy(B11->data[i], B->data[i], colHf * sizeof(field_t));
    memcpy(B12->data[i], &(B->data[i][colHf]),
           (colHf - colRem) * sizeof(field_t));
  }
  for (i = 0; i < midRCf - midRCRem; i++) {
    memcpy(B21->data[i], B->data[midRCf + i], colHf * sizeof(field_t));
    memcpy(B22->data[i], &(B->data[midRCf + i][colHf]),
           (colHf - colRem) * sizeof(field_t));
  }
...
int matrix_inv_strassen(matrix_t A, matrix_t dest, int m) {
  ...
  if (row < STRAINVCONST)
    return matrix_inv_standard(A, dest, m);
  ...
  for (i = 0; i < rowHf; i++) {
    ...
    memcpy(A12->data[i], &(A->data[i][rowHf]),
           (rowHf - rowRem) * sizeof(field_t));
  }
  for (i = 0; i < rowHf - rowRem; i++) {
    memcpy(A21->data[i], A->data[rowHf + i], rowHf * sizeof(field_t));
    memcpy(A22->data[i], &(A->data[rowHf + i][rowHf]),
           (rowHf - rowRem) * sizeof(field_t));
  }
  if (rowRem > 0)
    A22->data[rowHf - 1][rowHf - 1] = 1;
  ...
  for (i = 0; i < rowHf; i++) {
    ...
    memcpy(&(dest->data[i][rowHf]), A21->data[i],
           (rowHf - rowRem) * sizeof(field_t));
  }
  for (i = 0; i < rowHf - rowRem; i++) {
    memcpy(dest->data[rowHf + i], M4->data[i], rowHf * sizeof(field_t));
    memcpy(&(dest->data[rowHf + i][rowHf]), A22->data[i],
           (rowHf - rowRem) * sizeof(field_t));
  }
  matrix_free(A11);
  matrix_free(A12);
...
int matrix_vec_mat_mul(field_t V[], int vsize, matrix_t B, field_t dest[],
                       int dsize, int m) {
  switch (WINOGRADVEC) {
  case 0:
...
int matrix_mul(matrix_t A, matrix_t B, matrix_t C, int m) {
  ...
  case 2:
    return matrix_mul_winograd(A, B, C, m);
  default:
    return matrix_standard_mul(A, B, C, m);
  }
  return 0;
