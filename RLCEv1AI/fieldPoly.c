...
void poly_clear(poly_t p) {
  memset(&(p->coeff[1 + p->deg]), 0,
         (p->size - 1 - (p->deg)) * sizeof(field_t));
}
...
int poly_mul_karatsuba(poly_t f, poly_t g, poly_t r, int m) {
  ...
  if (mindeg < 35)
    return poly_mul_standard(f, g, r, m);
  middeg = 1 + maxdeg / 2;
  ...
  if ((1 + f->deg - middeg) > 0)
    memcpy(f1->coeff, &(f->coeff[middeg]),
           (1 + f->deg - middeg) * sizeof(field_t));
  memcpy(f2->coeff, f->coeff, middeg * sizeof(field_t));
  if ((1 + g->deg - middeg) > 0)
    memcpy(g1->coeff, &(g->coeff[middeg]),
           (1 + g->deg - middeg) * sizeof(field_t));
...
int poly_mul_FFT(poly_t f, poly_t g, poly_t r, int m) {
  ...
  if (deg >= r->size)
    return POLYMULTERRR;
  if (((f->deg) < 30) || ((g->deg) < 30))
    poly_mul_standard(f, g, r, m);
  int i, ret, d = 1;
  matrix_t smat =
      matrix_init(m - 1, m); // smat[i][j]=GF_log(s[i]->coeff[2^j],m);
  ...
  for (i = 0; i < output3->size; i++)
    output3->data[i] = GF_mul(output1->data[i], output2->data[i], m);
  poly_zero(r);
...
int poly_mul_FFT_fullField(poly_t f, poly_t g, poly_t r, int m) {
  if ((f->deg + g->deg) >= r->size)
    return POLYMULTERRR;
  if (((f->deg) < 100) || ((g->deg) < 100))
    poly_mul_standard(f, g, r, m);
  int i, ret;
  vector_t base, output1, output2, output3;
  base = vec_init(m);
  ...
  for (i = 1; i < fieldSize(m) - 1; i++)
    R->coeff[fieldSize(m) - 1 - i] = output3->data[GF_exp(i, m)];
  poly_deg(R);
  ret = FFT(R, output3, base, m);
  if (ret < 0)
    return ret;
  poly_zero(r);
  for (i = 0; i <= (f->deg) + (g->deg); i++)
    r->coeff[i] = output3->data[GF_exp(i, m)];
  poly_deg(r);
...
#ifdef __AVX2__
#include <immintrin.h>
// Helper: Multiply vector by scalar using precomputed tables (in-place)
static inline void GF_mul_update_avx2(__m256i *v0, __m256i *v1, __m256i TL0,
                                      __m256i TH0, __m256i TL1, __m256i TH1,
                                      __m256i TL2, __m256i TH2, __m256i TL3,
                                      __m256i TH3) {
  __m256i mask_low = _mm256_set1_epi16(0x00FF);
  __m256i mask_nibble = _mm256_set1_epi8(0x0F);

  // Process v0
  __m256i ve = _mm256_and_si256(*v0, mask_low);
  __m256i vo = _mm256_srli_epi16(*v0, 8);
  ...
  *v0 = _mm256_xor_si256(res_lo, _mm256_slli_epi16(res_hi, 8));

  // Process v1
  ve = _mm256_and_si256(*v1, mask_low);
  vo = _mm256_srli_epi16(*v1, 8);
  ve_n0 = _mm256_and_si256(ve, mask_nibble);
  ve_n1 = _mm256_and_si256(_mm256_srli_epi16(ve, 4), mask_nibble);
  vo_n0 = _mm256_and_si256(vo, mask_nibble);
  vo_n1 = _mm256_and_si256(_mm256_srli_epi16(vo, 4), mask_nibble);
  res_lo = _mm256_xor_si256(_mm256_xor_si256(_mm256_shuffle_epi8(TL0, ve_n0),
                                             _mm256_shuffle_epi8(TL1, ve_n1)),
                            _mm256_xor_si256(_mm256_shuffle_epi8(TL2, vo_n0),
                                             _mm256_shuffle_epi8(TL3, vo_n1)));
  res_hi = _mm256_xor_si256(_mm256_xor_si256(_mm256_shuffle_epi8(TH0, ve_n0),
                                             _mm256_shuffle_epi8(TH1, ve_n1)),
                            _mm256_xor_si256(_mm256_shuffle_epi8(TH2, vo_n0),
                                             _mm256_shuffle_epi8(TH3, vo_n1)));
  *v1 = _mm256_xor_si256(res_lo, _mm256_slli_epi16(res_hi, 8));
}

int find_roots_Chien_avx2(poly_t lambda, field_t lambdaRoots[],
                          field_t eLocation[], int m) {
  int fSize = fieldSize(m);
  field_t *accumulator = calloc(fSize + 32, sizeof(field_t)); // +padding

  // Initialize accumulator with term 0
  field_t c0 = lambda->coeff[0];
  if (c0 != 0) {
  ...
  // For each subsequent term c_i * x^i
  for (int i = 1; i <= lambda->deg; i++) {
    field_t ci = lambda->coeff[i];
    if (ci == 0)
      continue;

    // Calculate Multiplier stride for 32 elements: alpha^(i * 32)
    // alpha corresponds to exponent 1. alpha^x corresponds to exponent x.
    // We use GFexpTable/log to finding power.
    // alpha_log = 1.
    // multiplier_log = i.
    // block_multiplier_log = (i * 32) % fieldOrder[m].
    field_t block_mul_val = GFexpTable[m][(i * 32) % fieldOrder[m]];

    // Prepare initial block of 32 coefficients: [ci*a^(i*0), ci*a^(i*1)...]
    field_t current_block[32] __attribute__((aligned(32)));
    ;
    int log_ci = GFlogTable[m][ci];
    for (int k = 0; k < 32; k++) {
      int log_val = (log_ci + i * k) % fieldOrder[m];
      current_block[k] = GFexpTable[m][log_val];
    }

    // Prepare AVX Tables for block_mul_val
    __m256i TL0, TH0, TL1, TH1, TL2, TH2, TL3, TH3;
    GF_prepare_avx2_mult_tables(block_mul_val, m, &TL0, &TH0, &TL1, &TH1, &TL2,
                                &TH2, &TL3, &TH3);

    __m256i v0 = _mm256_load_si256((__m256i *)&current_block[0]);
    __m256i v1 = _mm256_load_si256((__m256i *)&current_block[16]);

    int num_blocks = fSize / 32;
    // Process blocks
    for (int b = 0; b < num_blocks; b++) {
      // Load Accumulator
      __m256i acc0 = _mm256_loadu_si256((__m256i *)&accumulator[b * 32]);
      __m256i acc1 = _mm256_loadu_si256((__m256i *)&accumulator[b * 32 + 16]);

      // XOR with current term
      acc0 = _mm256_xor_si256(acc0, v0);
      acc1 = _mm256_xor_si256(acc1, v1);

      // Store back
      _mm256_storeu_si256((__m256i *)&accumulator[b * 32], acc0);
      _mm256_storeu_si256((__m256i *)&accumulator[b * 32 + 16], acc1);

      // Update current term block for next iteration (multiply by alpha^(i*32))
      GF_mul_update_avx2(&v0, &v1, TL0, TH0, TL1, TH1, TL2, TH2, TL3, TH3);
    }
  }
  ...
  for (int j = 0; j < fSize; j++) {
    if (accumulator[j] == 0) {
      lambdaRoots[root_count] = j;
      if (j == 0)
        eLocation[root_count] = 0;
      else
        eLocation[root_count] = fSize - 1 - j;
      root_count++;
    }
  }

  // Original code did GF_expvec(lambdaRoots, i, m) at the end!
  // Why? "lambdaRoots[i]=j" where j is index (exponent).
  // The outputs are field elements.
  GF_expvec(lambdaRoots, root_count, m);
...
int find_roots_Chien(poly_t lambda, field_t lambdaRoots[], field_t eLocation[],
                     int m) {
#ifdef __AVX2__
  if (GFMULTAB == 1) { // AVX optimization depends on MultiTable logic
    return find_roots_Chien_avx2(lambda, lambdaRoots, eLocation, m);
  }
#endif
  ...
  for (j = 0; j < fieldSize(m); j++)
    mat->data[0][j] = lambda->coeff[0];
  for (i = 1; i < mat->numR; i++)
    mat->data[i][0] = lambda->coeff[i];
  for (i = 1; i < mat->numR; i++) {
...
int find_roots(poly_t p, field_t roots[], field_t eLocation[], int m) {
  ...
  if (p->deg <= 4) {
    ...
    if (ROOTFINDING == 0)
      return numRoots = find_roots_Chien(p, roots, eLocation, m);
    if (ROOTFINDING == 1)
      numRoots = find_roots_exhaustive(p, roots, m);
    if (ROOTFINDING == 2)
      numRoots = find_roots_BTA(p, roots, m);
    if (ROOTFINDING == 3)
      numRoots = find_roots_FFT(p, roots, m);
  }
...
int poly_mul(poly_t p, poly_t q, poly_t r, int m) {
  ...
  if (mindeg < 115)
    return poly_mul_standard(p, q, r, m);
  if (KARATSUBA == 0)
    return poly_mul_standard(p, q, r, m);
  if (KARATSUBA == 1)
    return poly_mul_karatsuba(p, q, r, m);
  if (KARATSUBA == 2)
    return poly_mul_FFT(p, q, r, m);
  return 0;
}
