...
int RLCEspad(unsigned char bytes[],size_t BLen,
	     unsigned char padded[], size_t paddedLen,
	     RLCE_public_key_t pk,
	     unsigned char randomness[], size_t randLen,
	     unsigned char e0[], size_t e0Len);
int RLCEspadDecode(unsigned char encoded[],size_t encodedLen,
		   unsigned char message[], unsigned long long *mlen,
		   RLCE_private_key_t sk,
		   unsigned char e0[], size_t e0Len);

int RLCEpad(unsigned char bytes[],size_t bytesLen,
	    unsigned char padded[], size_t paddedLen,
	    RLCE_public_key_t pk,
	    unsigned char randomness[], size_t randLen,
	    unsigned char e0[], size_t e0Len);
int RLCEpadDecode(unsigned char encoded[],size_t encodedLen,
		  unsigned char message[], unsigned long long *mlen,
		  RLCE_private_key_t sk,
		  unsigned char e0[], size_t e0Len);
int rangeadd(unsigned char bytes1[], unsigned char bytes2[], size_t bytesize);
...
int sk2B (RLCE_private_key_t sk, unsigned char skB[], size_t *blen) {
  ...
  if (invSLen>0) {
    for (a=0;a<(sk->S)->numR; a++) {
      memcpy(&(FE->data[j]),(sk->S)->data[a],((sk->S)->numC)*sizeof(field_t));
      j=j+(sk->S)->numC;
    }
  }
...
RLCE_private_key_t B2sk(const unsigned char binByte[], unsigned long long blen) {
  ...
  unsigned long long int sklen =sk->para[17];
  if (blen<sklen) {
    RLCE_free_sk(sk);
    return NULL;
  }
...
int RLCE_key_setup (unsigned char entropy[], int entropylen,
  ...
  generator=genPolyTable(n-k); //getGenPoly(n-k, generator, m);
  matrix_t G= NULL;
  matrix_t G2= NULL;
  if (OPTIMIZED==1) {
...
int RLCE_encrypt(unsigned char msg[], unsigned char entropy[], size_t entropylen,
  ...
  while (repeat) { /* this loop makes sure that errors will not be zero */
    repeat = 0;    
    if (CTRPADDRBG==0) hash_DRBG_Generate(drbgState, drbgInput,randBytes,nRB);
    if (CTRPADDRBG==1) ctr_DRBG_Generate(ctrdrbgState,drbgInput,randBytes,nRB);    
...
int RLCE_decrypt(unsigned char cipher[], unsigned long long clen, RLCE_private_key_t sk, unsigned char msg[],
  ...
  poly_deg(code);
  decodedWord=rs_decode(code, codeLen, codeDim, eLocation,m);
  poly_free(code);
  free(cp);
...
int RLCEspad(unsigned char bytes[],size_t bytesLen,
	     unsigned char padded[], size_t paddedLen,
	     RLCE_public_key_t pk,
	     unsigned char randomness[], size_t randLen,
	     unsigned char e0[], size_t e0Len) {
